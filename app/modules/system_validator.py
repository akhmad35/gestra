...
def validate_base64_image(image_data: str) -> Tuple[bool, Optional[bytes], str]:
    ...
    try:
        ...
        if len(decoded_bytes) == 0:
            return False, None, "Base64 image berhasil didecode tetapi menghasilkan 0 byte."
...
def decode_and_validate_canvas(decoded_bytes: bytes) -> Tuple[bool, Optional[np.ndarray], str]:
    """
...
def validate_domain_match(target: str, mode: str) -> Tuple[bool, str]:
    ...
    if mode_clean not in DOMAINS:
        return False, f"Mode '{mode}' tidak dikenal. Domain yang tersedia: {list(DOMAINS.keys())}."
    ...
    if char_to_check in valid_set:
        ...
        return False, f"Model tidak cocok dengan input. Karakter '{char_to_check}' tidak berada dalam domain '{mode_clean}'."
...
def evaluate_model_output(
    ...
    """
    Validasi 4 & 5: Melakukan evaluasi output model, menentukan status dan pesan edukasi.
    Confidence score input berkisar antara 0.0 - 1.0.
    """
    target_char = str(target).strip() if target else ""
    pred_clean = str(prediction).strip()
    
    # 1. Cek kecocokan domain
    domain_match, domain_msg = validate_domain_match(target_char, mode)
    if not domain_match:
        status = "Label tidak valid dalam model"
        message = "Coba lagi ya 😊"
        return {
            "status": status,
            "message": message,
            "correct": False,
            "explanation": f"Domain mismatch: {domain_msg}"
        }
        
    # Jika kanvas terdeteksi kosong
    if pred_clean == "Kosong" or confidence == 0.0:
        return {
            "status": "Tidak Yakin, coba ulang",
            "message": "Coba lagi ya 😊",
            "correct": False,
            "explanation": "Kanvas terdeteksi kosong atau coretan terlalu sedikit."
        }

    # 2. Aturan Evaluasi Confidence
    if confidence >= 0.8:
        status = "Benar"
        message = "Bagus, sedikit lagi ✍️"
    ...
    # Sinkronisasi logika is_correct
    # Jika target disuplai, pastikan prediksi cocok dengan target secara case-insensitive
    is_correct = False
    if target_char:
        is_correct = (pred_clean.lower() == target_char.lower()) and (status == "Benar")
    else:
    ...
    return {
        ...
        "message": message,
        "correct": is_correct,
        "explanation": f"Evaluasi berhasil dengan status '{status}' (confidence: {confidence:.2f})"
    }
...
def get_failsafe_response(message: Optional[str] = None) -> Dict[str, Any]:
    ...
    """
    fallback_msg = message if message else "Coba ulangi ya 😊 sistem sedang menyesuaikan input"
    return {
        "prediction": "unknown",
        "confidence": 0.0,
        "status": "system_error",
...
def run_pipeline_validation(
    ...
    """
    logger.info(f"[Validator] Menjalankan validasi pipeline untuk target='{target}', mode='{mode}'")
